...
class CorticalGrid:
    ...
    def flatten(self, arr_2d: np.ndarray) -> np.ndarray:
        """Reshape (n_rows, n_cols) array to (N,)."""
        return np.asarray(arr_2d).ravel()

    def unflatten(self, arr_flat: np.ndarray) -> np.ndarray:
        """Reshape (N,) array to (n_rows, n_cols)."""
        return np.asarray(arr_flat).reshape(self.shape)

    def pairwise_distances(self) -> np.ndarray:
        """Euclidean distance and displacement matrices (N x N)."""
        r = np.arange(self.N, dtype=float) // self.n_cols
        c = np.arange(self.N, dtype=float) % self.n_cols
